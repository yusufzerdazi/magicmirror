...
run_gendj() {
  ...
  if [ ! -d ".git" ]; then
    ...
    sed -i "s|const WEBSOCKET_URL = \"ws://0.0.0.0:8765\";|const WEBSOCKET_URL = \"wss://${RUNPOD_POD_ID}-8766.proxy.runpod.net\";|" ./fe/main.js
    sed -i "s|const PROMPT_ENDPOINT_URL_BASE = \"http://0.0.0.0:5556/prompt/\";|const PROMPT_ENDPOINT_URL_BASE = \"https://${RUNPOD_POD_ID}-5559.proxy.runpod.net/prompt/\";|" ./fe/main.js
